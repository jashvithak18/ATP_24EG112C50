...
//route to write article(protected route)
...
//send res
res.status(201).json("Article published successfully")
})
...
authorApp.put("/article",verifyToken("AUTHOR"),async(req,res)=>{
    ...
    const authorIdOfToken=req.user?.id
    //get modified article from author
    ...
    const modifiedArticle=await ArticleModel.findOneAndUpdate(
        ...
        if(!modifiedArticle){
            res.status(403).json({message:"Not authorized to edit the article"})
        }
        //send res
     res.status(200).json({message:"Article modified",payload:modifiedArticle})   
})

//delete article by id(soft delete)
